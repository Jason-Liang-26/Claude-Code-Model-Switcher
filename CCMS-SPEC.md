...
| `--diagnose` | 检查用户配置是否符合最佳实践 | 状态 + 交互式修复 |
| `--help` | 帮助 | 文本 |
...
|------|------|
| 查看所有凭据 | 列出所有模型的凭据状态 |
| 配置诊断 | 检查 `~/.claude/settings.json` 是否符合最佳实践 |
| 退出 | 退出程序 |

## 配置诊断

`--diagnose`（CLI）或菜单"配置诊断"检查 `~/.claude/settings.json` 是否符合最佳实践。

一期检查项：

| ID | 检查 | 期望值 | 影响 |
|----|------|--------|------|
| `attribution-header` | `env.CLAUDE_CODE_ATTRIBUTION_HEADER` | `"0"` | 禁用归属块（客户端版本 + prompt fingerprint），改善通过 LLM 网关路由时的 prompt caching 命中率。官方文档：设置为 0 以从系统提示的开头省略归属块。 |

检查不通过时交互式询问是否修复（写入 `~/.claude/settings.json`）。诊断规则通过 `_DIAGNOSTIC_RULES` 列表注册，未来扩展只需 append 新 dict。
